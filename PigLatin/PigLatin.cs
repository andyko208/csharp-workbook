...
namespace PigLatin
{
    class Program
    {
        ...
        {
            int firstVowelIndex = word.IndexOfAny(new char[] {'a', 'i', 'u', 'e', 'o'});
            int yChecker = word.IndexOfAny(new char[] {'y'});   //separate var checks if 'y' works as a vowel
            String part = "";   //the part that gets cut out
            if(firstVowelIndex == 0)   //#1 condition when first letter = vowel
            {
                part = word.Substring(0, firstVowelIndex);
                word = word.Substring(firstVowelIndex);
                word += part + "yay";
            }
            else if(yChecker >= 1 && firstVowelIndex > yChecker) //#2 condition when 'y' is used as a vowel
            {                                                     //only works when 'y' is before any other vowels
                part = word.Substring(0, yChecker);
                word = word.Substring(yChecker);
                word += part + "ay";
            }
            else if(firstVowelIndex >= 1)                //#3 condition when vowels, not 'y', exists but not 'y'
            {
                part = word.Substring(0, firstVowelIndex);
                word = word.Substring(firstVowelIndex);
                word += part + "ay";
